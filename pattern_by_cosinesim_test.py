...
def cosine_similarity(x, y):
    ...
    return round(numerator / float(denominator), 3)  # Cosine Similarity
...
for i in range(n):
    ...
    test_int = list(map(int, test))  # 의미 있는 1을 구하려고 일단 새로운 cluster을 정렬하고  첫번째 패턴을 0 1 0 1 나눔
    ...
    for i, v in enumerate(keyList):
        ...
        test_next_int = list(map(int, test_next))

        for m in range(len(test_int)):

            if ((test_int[m] == test_next_int[m]) & test_int[m] == 1):
    ...
    for p in range(len(test_int)):
        if(test_int[p]==1):
            key_for_common = key_for_common + "1 "
        else:
            key_for_common = key_for_common+ "0 "

    number_of_1 = test_int.count(1)   # 1의 개수 세주기

    common_1_dict[cluster_number]=key_for_common

    with open("test_cluster/" + 'cluster-' + str(cluster_number) + '-id-' + str(
            number_of_id) + '-number_of_value-' + str(number_of_1) + '.csv', 'wb') as csv_file:
        writer = csv.writer(csv_file)
        for key, value in new_dict_for_clustering.items():
            writer.writerow([key, value])
    cluster_number += 1
with open('common_pattern_test.csv', 'wb') as csv_file1:
